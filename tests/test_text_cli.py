"""Phase 1/2/3 checkpoints for the REPL: tools are actually wired into the
...
The one live test (gated on a real ANTHROPIC_API_KEY) is the full-pipeline
version of Phase 3's checkpoint: a genuinely uncovered policy question, run
through the actual Agent + real Claude + real search_policy (local
embedding backend — no Voyage key needed), checking the model doesn't
invent an answer once retrieval correctly comes back empty.
"""
...
async def test_search_policy_tool_is_wired_into_the_loop(monkeypatch):
    ...
    assert result.tool_calls == [
        {"name": "search_policy", "input": {"query": "How long do I have to return something?"}}
    ]
