# Phase 12 — Escalation as a Resolvable Process (v4)
...
**Supersedes:** v1, v2 and v3. **Do not implement any of them.** v1 deleted `end_reason="escalated"` and silently killed Phase 10d. v2 kept the string but broke both its consumers, rested on a fabricated constraint, and left the original bug intact. What each got wrong is recorded below, because the same mistakes are easy to re-make.
...
async def test_a_spoken_question_reaches_the_model_history(monkeypatch):
    ...
    assert len(session.agent.messages) == before, "no extra message may be appended"
...
## Corrections applied from the v3 review

| Finding | Fix |
|---|---|
| CRIT-1 — end suppression had no budget term, trapping the customer forever | D-11; the give-up test now drives `run_turn` |
| CRIT-2 — an offer hung up on its own question | D-11's first branch; new test scripts `end_conversation` on the offer turn |
| HIGH-1 — `mandatory` derivation downgraded high-value refunds | D-12; new test asserts a *dynamic* tool reason is mandatory |
| HIGH-2 — turn accessor unspecified | D-13 dictates `SessionGates.turn` |
| HIGH-3 — dead `RESOLUTION_TRANSFER` branch | D-14; branch deleted, defect flagged, test removed |
| HIGH-4 — history test passed on a call-killing append | D-15; asserts message count and content shape |
| HIGH-5 — failed `open_escalation` discarded the escalation | D-16; state opens before it persists |
| HIGH-6 — eval framework cannot express an offer | `eval/harness.py` + `eval/scoring.py` added to scope |
| Admitted #2 — `messages` accessor | Dictated: zero-argument accessor, with the reason |
