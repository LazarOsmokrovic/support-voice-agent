# Phase 12 — Escalation as a Resolvable Process (v5)
...
**Supersedes:** v1, v2, v3 and v4. **Do not implement any of them.** v1 deleted `end_reason="escalated"` and silently killed Phase 10d. v2 kept the string but broke both its consumers, rested on a fabricated constraint, and left the original bug intact. What each got wrong is recorded below, because the same mistakes are easy to re-make.
...
- **Touch only these files:**
  ...
  `observability/turn_log.py`, `eval/scenarios.py`, `eval/harness.py`, `eval/scoring.py`,
  and their tests.

  The last four are **not** scope creep. Without `turn_log.py` this phase silently changes
  what `escalation_reason` means. Without `scenarios.py` six eval scenarios break. And
  without `harness.py` + `scoring.py` the eval framework cannot express "the agent made an
  offer" at all — `ObservedTurn` (`eval/harness.py:54-60`) and `score_escalation`
  (`eval/scoring.py:87-119`) both read `escalation_reason` and nothing else, so the three
  suggested-trigger scenarios could only assert "nothing happened", which passes identically
  if the agent silently did nothing. That is the phase's most valuable new coverage; it is
  worth two small files. See Task 7.
...
v3 was reviewed by an independent engineer and did not survive it either: 2 critical,
6 high, 11 medium. Its **architecture verified** — D-2, D-3, D-4 and D-8 all check out
against the code, and every line reference is correct. The defects were in blocks written
by hand, and in tests that agreed with their own bugs.

**CRIT-1 — the end suppression had no budget term.** v3 dictated
`if ending and state.is_open: ending = False`. The refusal budget governed only the tool's
*message*; after `MAX_END_REFUSALS` the tool returns "Session marked complete." but
`state.is_open` is still True, so the end was suppressed **forever**. A customer who just
wanted to leave could not. v3's own test for this called `end_conversation` and
`should_end_session` directly and never drove `run_turn`, so it passed with the trap
present. **Fix: D-11.**

**CRIT-2 — a suggested trigger reproduced the originating bug one branch over.** An offer
leaves `status == none`, so nothing suppressed the ending: the agent could say "Would it
help if I arranged for a colleague to call you back?" and hang up on the same turn. That is
the spec's originating transcript. Its test used a plain-text reply with no
`end_conversation` call — the exact v2 mistake v3's own opening section called out.
**Fix: D-11.**

**HIGH-1 — `mandatory` was never dictated, and the obvious derivation breaks refunds.**
`agent/tools/refunds.py:185` returns a *dynamic* reason,
`f"high-value refund (${amount:.2f}) requires specialist approval"`, which can never be a
member of a static `MANDATORY_REASONS`. An implementer writing
`mandatory = reason in MANDATORY_REASONS` downgrades "a specialist must approve" to "would
you like a human?", and breaks `eval/scenarios.py:314`. **Fix: D-12.**

**HIGH-2 — the turn accessor was left to the implementer and must not be.** A value that
does not advance per turn makes `consume_refusal` return True forever — CRIT-1's outcome by
another route, with no test in the plan able to detect it. **Fix: D-13.**

**HIGH-3 — the DTMF double-notification "fix" was dead code.** Nothing ever sets
`RESOLUTION_TRANSFER` on `gates.escalation`, because the DTMF path bypasses `run_turn`
entirely. **Fix: D-14, which invokes this plan's own rule-5 escape hatch rather than
pretending otherwise.**

**HIGH-4 — the history test passed on a call-killing implementation.**
`assert outcome.notice in str(messages)` also passes when the notice is appended as a NEW
assistant message, producing `assistant, assistant, user` — which the Messages API rejects,
so the *next* turn of the call raises. **Fix: D-15.**

**HIGH-5 — a failed `open_escalation` silently discarded the escalation.** v3 called
`state.open()` only after a successful await, so on failure there was no row, no notice, no
suppression, and `end_reason` fell back to `model_ended` — meaning the Twilio hook does
**not** fire. Today's code still returns `"escalated"` on that path and the caller reaches a
human. A regression on the failure path of the most safety-relevant trigger. **Fix: D-16.**
...
**D-11 — The end is suppressed for TWO distinct reasons, and the insistence is bounded.**

v3 checked one condition and had two holes. The rule is:

```python
ending = should_end_session(result.tool_calls)
if ending and notice and notice.rstrip().endswith("?"):
    # We just asked the customer something. Never hang up on our own question —
    # that is the spec's originating transcript, and it is reachable from the
    # OFFER branch too, where status is still `none` and an is_open check sees
    # nothing. Costs no refusal budget: the agent has not insisted yet, it has
    # only asked once.
    ending = False
    warnings.append("end suppressed: the agent asked a question this turn")
elif ending and state.is_open and state.consume_refusal(session.turn):
    # A handover is open and the agent may still insist. consume_refusal returns
    # False once MAX_END_REFUSALS turns have been spent, and then this branch
    # stops firing and the customer leaves. A refusal is a nudge, not a cage.
    ending = False
    warnings.append("end suppressed: the handover is still open")
```

The two `consume_refusal` call sites — `end_conversation`'s handler during the tool loop,
and this one after it — are consistent by construction: the per-turn guard means the second
call in the same turn neither increments nor disagrees with the first.

**D-12 — `mandatory` is set explicitly at each return site. `MANDATORY_REASONS` is never
used to derive it.**

`_tool_signaled_escalation` returns a reason built by the *tool*
(`agent/tools/refunds.py:185` interpolates the amount), so no static set can contain it.
`MANDATORY_REASONS` and `SUGGESTED_REASONS` exist for tests, eval scoring and readers —
never as the mechanism. A tool asking for a specialist is a rule, not an inference, and it
is mandatory regardless of how its sentence is worded.

**D-13 — The refusal budget reads `SessionGates.turn`.**

`SessionGates` gains `turn: int = 0`, incremented in `advance_turn` alongside the two gates.
The `end_conversation` handler closes over `gates`. **Not** `gates.scheduling.turn`:
`Session.turn`'s own comment (`agent/session.py:177-184`) explains why borrowing a
`PendingActionGate` counter for an unrelated purpose is a trap, and that reasoning applies
here unchanged.

**D-14 — The DTMF double-notification is NOT fixed in this phase, and no code pretends it is.**

A caller with an open handover who then presses 0 gets two Slack messages: the transfer
(from `create_handoff_packet`) and, at close, the session's own handover as `unresolved`.
Fixing it needs `EscalationState.mark_transferred()` called from
`transport/pipecat_processors.py` — a `transport/` edit, which this plan's Global
Constraints forbid.

So this plan **invokes its own escape hatch**: the abstraction genuinely has a hole here
(`_handle_dtmf_escalation` bypasses `run_turn` entirely, which is why Phase 9 gave it its
own turn-log write), and that is exactly the case CLAUDE.md rule 5 says to stop and flag
rather than route around. **Flag it to the project owner and let them decide.** Meanwhile:
delete the dead `RESOLUTION_TRANSFER` branch from `close_session`, and do not write
`test_a_transferred_caller_is_not_also_reported_unresolved` — a test for behaviour this
phase does not implement is worse than no test. Record it in `PROGRESS.md` as a known
limitation.

**D-15 — The notice is appended to the LAST assistant message, never as a new one.**

Appending a new `{"role": "assistant"}` message produces `assistant, assistant, user` once
`Agent.send` (`agent/core.py:114`) adds the next turn. The Messages API rejects
non-alternating roles, so the *next* turn raises and the call dies — the customer answers
the question and the bot crashes. `_append_to_last_assistant_message` edits the existing
message's content in place, the same shape `_substitute_hedge_in_history`
(`agent/session.py:129`) already uses safely. Tests assert the message COUNT is unchanged,
not that the text appears somewhere in `str(messages)`.

**D-16 — State opens before it persists.**

`state.open(reason)` is in-memory and cannot fail; `open_escalation` is a network call that
can. Open first, then persist inside `try`. On failure the handover is still open, the
notice is still spoken, the end is still suppressed, and `pending_persist` stays True so
`close_session` retries — instead of v3's outcome, where a failed packet build silently
produced no handover at all and let a "get me a human" call end as `model_ended`, skipping
the Twilio transfer that today still happens.
...
| `observability/turn_log.py` | `escalation_offered` field, declared LAST with a default. |
| `eval/harness.py` | `ObservedTurn.escalation_offered`, populated from the turn record. |
| `eval/scoring.py` | `score_offer`, wired into `score_expectations` and `score_drift`. |
| `eval/scenarios.py` | `Expectations.offer_turn` / `offer_reason`; rewrite six scenarios. |
...
Four categories of existing-test breakage, all to be fixed rather than weakened. **Do not
weaken an assertion to make it pass** — a test that still passes while comparing a dataclass
to a string has stopped checking anything.

1. **Tests comparing `check_escalation` / `record_turn` to a string** → read `signal.reason`.
2. **Stubs that RETURN a string.** `tests/test_session.py:146` uses
   `AsyncMock(return_value="explicit request for a human")`. Under this plan
   `signal.mandatory` on a `str` raises `AttributeError`, outside run_turn's classifier
   `try`, so it kills the turn. Re-point at an `EscalationSignal`.
3. **Stubs of a function `run_turn` no longer calls.** `tests/test_session.py:113`, `:150`
   and `:470` stub `create_handoff_packet`; `run_turn` now calls `open_escalation`.
   **Re-point them — do not delete them.** Deleting makes the test construct a real
   `anthropic.AsyncAnthropic` and reach the network.
4. **Assertions on the old notice wording.** `tests/test_session.py:127` asserts
   `"call you back" in outcome.notice`; `_escalation_handover_notice()` says "give you a
   call". Update the expected text.
...
**Every test in this task takes `tmp_path` and repoints the database**, exactly as
`tests/test_escalation.py:161` and the Task 4/5 tests do:
...
4. `pending_confirmation` → `{"scheduled": False, "status": "pending_confirmation",
   "message": booking["message"]}`.
5. `not booked` → `{"scheduled": False, "error": booking["error"], "message":
   booking["message"]}`, so the model still sees `slot_unavailable`. **State stays open.**

**Every return from this tool carries `scheduled: bool`.** v4 said cases 4 and 5 passed
`book_appointment`'s dict through "unchanged" — but that dict carries `booked`, not
`scheduled`, so the plan's own test asserting both `result["scheduled"] is False` and
`result["status"] == "pending_confirmation"` would have raised `KeyError` on a compliant
implementation.
...
One helper, one name, used at both call sites. v4 defined `_may_open` (a pure predicate)
and then called `_ensure_open` (which by its name checks *and* mutates) — two contracts,
...
def _ensure_open(state: EscalationState) -> bool:
    ...
    Both tools are registered for every session, so without this guard a model
    could call one during an entirely ordinary conversation — "I'll get back to
    you when I know my schedule" is a plausible prompt — and manufacture an
    escalations row, a Slack message and a changed end_reason out of nothing.
    """
...
Both tools call it the same way:

```python
    if not _ensure_open(state):
        return {"scheduled": False, "error": "no_handover",
...
`messages` for `open_escalation`'s inference. It must be a **zero-argument accessor closing
over `gates`** (or a deferred lookup), never a direct reference: `build_dispatch_tool` runs
at `agent/session.py:199`, *before* `Agent` is constructed at `:200`, so there is no list to
capture at that point. `Agent.messages` (`agent/core.py:103`) is mutated in place and never
rebound, so a deferred lookup stays live for the whole session.
...
@pytest.mark.asyncio
async def test_the_refusal_gives_up_rather_than_trapping_the_customer(monkeypatch, tmp_path):
    ...
    MUST drive run_turn, not summary.end_conversation directly. v3's version
    of this test called the tool and should_end_session in a Python loop — and
    passed while run_turn's own suppression trapped the customer forever,
    because the suppression had no budget term and the test never reached it.
    The trap lives in run_turn, so the test has to go through run_turn."""
    ...
    await run_turn(session, "I want to speak to a person")        # opens; asks a question
...
def test_the_refusal_budget_is_consistent_across_its_two_call_sites():
    """consume_refusal is called twice per turn — once by end_conversation
    inside the tool loop, once by run_turn after it. The per-turn guard must
    make the second call agree with the first rather than double-spending."""
    state = EscalationState()
    state.open("explicit request for a human")

    for turn in range(1, escalation.MAX_END_REFUSALS + 1):
        assert state.consume_refusal(turn) is True, "the tool refuses"
        assert state.consume_refusal(turn) is True, "run_turn agrees, same turn"
...
`agent/session.py`: **D-13 dictates the mechanism — do not improvise one.** `SessionGates`
gains `turn: int = 0`, incremented in `advance_turn` beside the two gates:
...
and the handler closes over `gates`:
...
**Not** `gates.scheduling.turn` — `Session.turn`'s own comment (`agent/session.py:177-184`)
explains why borrowing a `PendingActionGate` counter for an unrelated purpose is a trap, and
that reasoning applies here unchanged. A turn number that does not advance makes
`consume_refusal` return True forever and traps the customer, with no unit test able to see
it — which is why this must be dictated rather than left open. `should_end_session`
...
**Files:** `agent/session.py`, `observability/turn_log.py`, `tests/test_session.py`,
...
async def test_a_suggested_trigger_after_a_booked_callback_neither_offers_nor_blocks_goodbye(
    ...
    # ... open a handover, resolve it as RESOLUTION_CALLBACK, then fire a
    # suggested signal on a turn that also calls end_conversation
...
async def test_a_handover_whose_packet_build_failed_is_still_recorded_at_close(
    ...
    already built."""
    # force open_escalation to raise once inside run_turn, then close
    assert rows == 1, "the handover must reach the database eventually"
    assert len(sent) == 1, "and the human must be told exactly once"


@pytest.mark.asyncio
async def test_an_abandoned_handover_is_reported_even_when_the_summary_fails(
    monkeypatch, tmp_path
):
    ...
    # ... open a handover, never resolve it, then close
...
# NOT in v4. Fixing that defect requires the DTMF handler to mark the session's
# state, which is a transport/ edit this plan forbids — so per D-14 it is
# flagged to the project owner instead. A test for behaviour the phase does not
# implement is worse than no test.
```
...
```python
    ...
    if signal and signal.mandatory:
        if state.status == escalation.STATUS_NONE:
            ...
            # network call that can. v3 opened only after a successful await,
            # so a failed packet build produced no handover at all: no notice,
            # no suppression, and end_reason fell back to model_ended, which
            # means transport/pipecat_processors.py:418 does NOT fire the warm
            # transfer. Today's code still returns "escalated" there and the
            # caller reaches a human. Losing that on the failure path of "I
            # want a human" would be the worst regression in this phase.
        ...
        # v4 re-offered a callback the customer had already settled — and
        # because that offer ends in "?", D-11's first branch then suppressed
        # their goodbye to wait for an answer to a question the agent has
        # already decided it will refuse (schedule_human_callback returns
        # already_resolved). The spec is explicit: one handover per session,
        # and after it is arranged the agent just asks what else it can help
        # with. Reachable on any post-resolution turn, and "sustained negative
        # sentiment" is likelier than not from someone who escalated.
    ...
    ending = should_end_session(result.tool_calls)
    if ending and notice and notice.rstrip().endswith("?"):
        # D-11, first reason: we just asked the customer something. Never hang
        # up on our own question. This branch is what catches the OFFER case,
        # where status is still `none` and an is_open check sees nothing —
        # v3's hole, and the spec's originating transcript one branch over.
        # Costs no refusal budget: asking once is not insisting.
        ending = False
        warnings.append("end suppressed: the agent asked a question this turn")
        ...
        # insist. consume_refusal returns False once MAX_END_REFUSALS turns are
        # spent, and then this branch stops firing and the customer leaves.
        # gates.turn, NOT session.turn — it must be the same counter the
        # end_conversation handler reads (D-13), or the two call sites can
        # disagree. They are different counters (agent/session.py:284-285)
        # and Session.turn is also incremented outside run_turn.
        # v3 omitted the budget term entirely, so an open handover suppressed
        # the end forever — a customer who just wanted to go could not.
        ending = False
        warnings.append("end suppressed: the handover is still open")
        if not notice:
            notice = _escalation_handover_notice()
...
`observability/turn_log.py` gains `escalation_offered: str | None = None`, declared as the
**LAST** field of `TurnRecord` — every preceding field is non-default, so it cannot go
anywhere else, and without the default the two other construction sites raise `TypeError` on
every error-path turn and every DTMF keypress. One of those sites is
`transport/pipecat_processors.py:249`, which this plan forbids editing; the default is what
keeps that true. Also pass `escalation_offered=None` explicitly at `agent/session.py:305`
(the turn-failure path), and add `"escalation_offered"` to the asserted field list in
`tests/test_turn_log.py:64-72` so the new column is proven to serialise.
...
**Placement is dictated, not left to the implementer.** The existing function returns from
both branches (`agent/session.py:462-474`), so a flush appended after them is unreachable,
and a flush placed *after* the summary call is skipped whenever the summary fails — which on
a teardown path (no credit, a 529) is the common case, not the exotic one. The spec is
explicit that an abandoned handover must always be reported. **Flush FIRST, then summarise,
...
async def close_session(session: Session) -> SessionCloseResult:
    ...
    # v4 guarded on the packet and so skipped the retry it promised, leaving a
    # "get me a human" call with no row, no notification and no way to ever
    # create one.
    state = session.gates.escalation
    if state.pending_persist and state.status != escalation.STATUS_NONE:
        # NOTE: no RESOLUTION_TRANSFER branch here. v3 had one and it was dead
        # code — nothing ever sets that resolution on gates.escalation, because
        # the DTMF path builds its own row and never touches session state. See
        # D-14: that defect is flagged to the owner, not silently "handled".
        try:
...
`SessionCloseResult` has a single `error` field (`agent/session.py:455-459`); joining is
deliberate, so a handover failure is never hidden behind a summary failure.
...
### Task 7: Teach the eval framework about offers, then rewrite the six scenarios

**Files:** `eval/harness.py`, `eval/scoring.py`, `eval/scenarios.py`, `tests/`

**Step 0 comes first and is not optional.** v4 argued at length that `eval/harness.py` and
`eval/scoring.py` had to be in scope, added them to the allow-list, recorded the fix in its
own summary table — and then no task touched either file. The scenarios in Group B were left
unable to express "an offer was made", so they reduced to `escalation_turn=None` plus "no
escalations row": identical in discriminating power to the existing
`triage_calm_conversation_never_escalates` (`eval/scenarios.py:456-469`), passing unchanged
if the agent makes no offer at all or the whole suggested-trigger branch is deleted. That is
this project's signature failure — a test that passes while the bug it names is present —
reintroduced at round four in the exact place the plan claimed to have eliminated it.

**Step 0 is what makes Group B mean anything. Write it before touching a scenario.**

- [ ] **Step 0a: `ObservedTurn` carries the offer**

`eval/harness.py:44-61` — add `escalation_offered: str | None = None`, and populate it from
the turn record where the other fields are read (`eval/harness.py:139-153`):

```python
            escalation_offered=record.escalation_offered,
```

- [ ] **Step 0b: `Expectations` can state one**

`eval/scenarios.py:85-107` — add, beside `escalation_turn` / `escalation_reason`:
...
```python
    ...
    # mandatory-trigger scenario needs.
    offer_turn: int | None = None
    offer_reason: str | None = None
```

- [ ] **Step 0c: `score_offer`, mirroring `score_escalation`**

`eval/scoring.py` — add a `score_offer(scenario, result)` shaped exactly like
`score_escalation` (`eval/scoring.py:87-119`) but reading `row.escalation_offered`, and call
it from `score_expectations` (`eval/scoring.py:289-300`). Add `"escalation_offered"` to the
key tuple in `score_drift` (`eval/scoring.py:317-325`) — without that, drift never checks it
and a regression in offers goes unnoticed between recordings.

- [ ] **Step 0d: prove the new scoring can fail**

A scorer that cannot fail is worth nothing. Add a test that builds a result whose observed
turns carry no offer, against a scenario expecting `offer_turn=2`, and asserts
`score_offer` reports a failure — and the mirror case that it passes when the offer is there.

---

**Then** the scenarios themselves.
...
- **Mediums addressed:** M-10 (`notice` init), M-11 (repurpose the helpers), M-12
  (`key_prefix`), M-13 (`_ensure_open`), M-14 (skip transfer at close), M-15 (idempotent +
...
- **Known soft spots:** Task 5 is the largest diff. Every mechanism v3 left to the
  implementer is now dictated (D-12, D-13, and the `messages` accessor in Task 3) — each of
  the two v3 left open turned out to have a wrong answer that traps a customer or silently
  downgrades a refund escalation.
- **Carried forward, not fixed:** the DTMF double-notification (D-14). It needs a
  `transport/` edit, so it is flagged to the owner under rule 5 rather than worked around.
  No test in this plan claims otherwise.

## Corrections applied from the v4 review

| Finding | Fix |
|---|---|
| **CRITICAL 1** — eval offer coverage claimed fixed, never implemented | Task 7 **Step 0a-0d**: real edits to `eval/harness.py`, `eval/scoring.py`, `eval/scenarios.py`, plus a test proving `score_offer` can fail. Both files now appear in the File Structure table. |
| HIGH 2 — suggested trigger re-offers after resolution and blocks goodbye | Offer gated on `STATUS_NONE`, not `not is_open`; new test scripts `end_conversation` on that turn |
| HIGH 3 — D-16's promised retry did not exist | `close_session` guards on the handover existing and **rebuilds a missing packet**; new test forces `open_escalation` to raise |
| HIGH 4 — flush placement unspecified; summary failure swallowed it | Flush **first**, then summarise, errors joined; test stubs the summary to raise |
| HIGH 5 — history test failed on a correct implementation | `== before + 2`, plus an explicit no-consecutive-assistants assertion |
| HIGH 6 — `escalation_offered` broke two construction sites | Declared **last** with a default; passed explicitly at the error path; `transport/` untouched |
| MED 7 — `_stub_infer_fields` could not build `HandoffFields` | Real strings; explicit warning not to widen the model |
| MED 8 — Task 2 tests wrote to the real database | `autouse` fixture repoints `DB_PATH`; `CUSTOMER_ID` read from the seed |
| MED 9 — two different turn counters | Both `consume_refusal` sites read `gates.turn` |
| MED 10 — structural eval tests examined nothing | Positive controls: `assert examined >= 3` / `>= 1` |
| MED 11 — return shape contradicted its own test; two helper names | Every return carries `scheduled`; one `_ensure_open` that guards and opens |
| MED 12 — only two of four test-breakage categories named | All four named, with the warning that deleting a stale stub reaches the network |

## Why there was a v5

Rounds two, three and four each failed review on the same thing: the plan **claimed** a fix
that no task implemented. v4's worst case was arguing for two files to be in scope, adding
them to the allow-list, recording it as done — and never writing a step that edits either.

A summary table is not the work. Before implementation starts, the check is: for every row
above, name the numbered step that does it. If there is no step, the row is a lie.
